...
def precheckin_share_url(env, kind, record_id, token, lang=None):
    ...
    ``lang`` is a folio language, i.e. a plain ``res.lang`` code such as
    ``es_ES``; the share module narrows it to the segment its route accepts and
    drops whatever it does not recognise, so it is handed over as it is.
    """
    model_name = "roomdoo.precheckin.share"
    if model_name not in env:
        return ""
    return env[model_name].get_public_share_url(
