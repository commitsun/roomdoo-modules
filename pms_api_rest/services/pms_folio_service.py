...
class PmsFolioService(Component):
    ...
    def get_folios(self, folio_search_param):  # noqa: C901
        domain_fields = []
        result_folios = []
        ...
        # ---------------------------------------------------------
        # Date range (by nights) -> translate to folio ids,
        # then filter reservations by folio_id
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # Dynamic parts (each element is a VALID domain)
        # We'll AND them all together at the end.
        # ---------------------------------------------------------
        parts = []
        today = fields.Date.today()

        if folio_search_param.last:
            parts.append([("checkin", ">=", today)])

        if folio_search_param.ids:
            parts.append([("folio_id", "in", folio_search_param.ids)])

        if folio_search_param.lastUpdateFrom:
            last_update_from = fields.Datetime.from_string(
                folio_search_param.lastUpdateFrom
            )
            parts.append([("write_date", ">=", last_update_from)])

        if folio_search_param.checkinDate:
            checkin_date = fields.Date.from_string(folio_search_param.checkinDate)
            parts.append([("checkin", "=", checkin_date)])

        if folio_search_param.checkoutDate:
            checkout_date = fields.Date.from_string(folio_search_param.checkoutDate)
            parts.append([("checkout", "=", checkout_date)])

        # These are simple AND conditions -> no expression.AND needed
        if folio_search_param.pendingCheckin:
            parts.append(
                [
        ...
        if folio_search_param.completedCheckin:
            parts.append(
                [
                    ("state", "=", "onboard"),
                    ("reservation_type", "!=", "out"),
        ...
        if folio_search_param.pendingCheckout:
            parts.append(
                [
        ...
        if folio_search_param.completedCheckout:
            parts.append(
                [
                    ("state", "=", "done"),
                    ("reservation_type", "!=", "out"),
        ...
        # Free-text filter: email OR (name/partner/mobile/external_reference...)
        if folio_search_param.filter:
            ...
            if "@" in target:
                ...
                subdomains = [
                    [("name", "ilike", target)],
                    [("partner_name", "ilike", spaced)],
                    [("checkin_partner_ids.name", "ilike", spaced)],
                    [("mobile", "ilike", target)],
                    [("external_reference", "ilike", target)],
                ]
                parts.append(expression.OR(subdomains))

        # filterByState (each case becomes ONE domain part)
        if folio_search_param.filterByState:
            fbs = folio_search_param.filterByState

            if fbs == "checkinYesterday":
                parts.append(
                    [
                        ("state", "in", ("confirm", "arrival_delayed")),
            ...
            elif fbs == "completedCheckinsToday":
                parts.append(
                    [
                        ("state", "=", "onboard"),
                        ("checkin", "=", today),
            ...
            elif fbs == "pendingCheckinsTomorrow":
                parts.append(
                    [
                        ("state", "=", "confirm"),
                        ("checkin", "=", today + timedelta(days=1)),
            ...
            elif fbs == "pendingCheckoutsToday":
                parts.append(
                    [
                        ("state", "not in", ("cancel", "done")),
                        ("checkout", "=", today),
            ...
            elif fbs == "pendingCheckoutsTomorrow":
                parts.append(
                    [
                        ("state", "not in", ("cancel", "done")),
            ...
            elif fbs == "completedCheckoutsToday":
                parts.append(
                    [
                        ("state", "=", "done"),
                        ("checkout", "=", today),
            ...
            elif fbs == "completedCheckoutsTomorrow":
                parts.append(
                    [
                        ("state", "=", "done"),
                        ("checkout", "=", today + timedelta(days=1)),
                ...
                parts.append([("state", "=", "cancel")])

            elif fbs == "overbooking":
                parts.append([("overbooking", "=", True)])

        # ---------------------------------------------------------
        # Final domain: AND of base fields + all parts
        # ---------------------------------------------------------
        domain = expression.AND([domain_fields] + parts) if parts else domain_fields

        # ---------------------------------------------------------
        # Query reservations -> folios
        # ---------------------------------------------------------
        reservations_result = (
    ...
    def create_folio_reservations(self, folio_id, folio_info):
        ...
        for reservation in folio_info.reservations:
            self.env["pms.reservation"].sudo().create(
                {
    ...
    def create_folio(self, pms_folio_info):  # noqa: C901
        pms_property = (
        ...
        try:
            if pms_folio_info.externalReference and external_app:
                ...
                # ignore the creation of the folio and log the payload
                # with "duplicate" mensaje
                folio = (
                    self.env["pms.folio"]
                    .sudo()
                    .search(
                ...
                if folio:
                    _logger.info(
                        "Folio with external reference %s"
                        " and property %s already exists",
                        pms_folio_info.externalReference,
                ...
                if reservation.services:
                    for service in reservation.services:
                        if service.serviceLines:
                            ...
                            # For per-day products, ExternalApps sends `priceUnit`
                            # as the total price for all days, so it must be
            ...
            # &
            # the sending of mail should be a specific call
            # once the folio has been created?
            ...
            if not external_app:
                raise ValidationError(_("Error creating folio from API: %s") % e) from e
            else:
                return 0
    ...
    def create_folio_invoices(self, folio_id, invoice_info):
        ...
        line_notes = folio.sale_line_ids.filtered(
            ...
            and line.section_id in sale_lines_to_invoice
        )
        ...
        for item in invoice_info.saleLines:
            if item.id in invoices.invoice_line_ids.mapped("folio_line_ids.id"):
                invoice_line = invoices.invoice_line_ids.filtered(
                    lambda r, _item=item: _item.id in r.folio_line_ids.ids
    ...
    def _get_section_qty_to_invoice(self, sale_line):
        ...
        if sale_line.display_type == "line_section":
            ...
            next_line_section = folio.sale_line_ids.filtered(
                lambda line: line.sequence > seq and line.display_type == "line_section"
            )
            if next_line_section:
                return sum(
                    folio.sale_line_ids.filtered(
                        lambda line: line.sequence > seq
                        and line.sequence < next_line_section[0].sequence
                        and line.display_type != "line_section"
    ...
    def update_folio_values(self, folio, pms_folio_info):  # noqa: C901
        external_app = self.env.user.pms_api_client
        folio_vals = {}
        if pms_folio_info.state == "cancel":
        ...
        if pms_folio_info.reservations:
            ...
            if reservations_vals:
                ...
                for val in reservations_vals:
                    # Cancel the old reservations that have not been
                    ...
                    if val[0] == 1:
                        if val[2].get("state") == "cancel":
                            ...
                            # delete from reservations_vals the reservation
    ...
    def normalize_payments_structure(self, pms_folio_info, folio):
        ...
        """
        if pms_folio_info.transactions:
            # If the payment issuer is the API client,
            # the payment will come in transactions
    ...
    def wrapper_reservations(self, folio, info_reservations):  # noqa: C901
        ...
        for info_reservation in info_reservations:
            # Search a reservation in saved_reservations whose sum of
            # night amounts is equal
            ...
            proposed_reservation = saved_reservations.filtered(
                lambda r, _ir=info_reservation: r.checkin
                == datetime.strptime(_ir.checkin, "%Y-%m-%d").date()
                and r.checkout == datetime.strptime(_ir.checkout, "%Y-%m-%d").date()
                and r.room_type_id.id == _ir.roomTypeId
                and r.adults == _ir.adults
                and r.children == _ir.children
                and r.state != "cancel"
            )
    ...
    def force_api_update_avail(
        ...
        It is used to override potential availability changes on the channel
        made unilaterally,
    ...
    # INTERNAL METHODS FOR PUBLIC ENDPOINT
    def _get_folio_or_404(self, folio_id):
        folio_record = self.env["pms.folio"].sudo().browse(folio_id)
        if not folio_record.exists():
            raise MissingError(_("Folio not found"))
        return folio_record

    def _check_folio_access(self, folio_record, token):
        try:
            CustomerPortal._document_check_access(
                self, "pms.folio", folio_record.id, access_token=token
            )
        except AccessError:
            raise MissingError(_("Folio not found")) from None
    ...
    def _get_reservation_access_token(self, reservation):
    ...
    def _get_folio_portal_link(self, folio_record):
        return (
            self.env["ir.config_parameter"].sudo().get_param("web.base.url")
            + folio_record.get_portal_url()
            if folio_record.get_portal_url()
            else ""
        )

    def _get_folio_checkin_names_completed(self, folio_record):
        return [
            checkin_partner.firstname
            for reservation in folio_record.reservation_ids
            for checkin_partner in reservation.checkin_partner_ids
    ...
    def get_folio_public_info(self, folio_id, token):
        ...
        folio_portal_link = self._get_folio_portal_link(folio_record)
        ine_category = (
            f"{folio_record.pms_property_id.ine_category_id.category}"
            f" ({folio_record.pms_property_id.ine_category_id.type})"
            if folio_record.pms_property_id.ine_category_id
            else ""
        )
        ...
        return self.env.datamodels["pms.folio.public.info"](
            ...
            folioCheckinNamesCompleted=self._get_folio_checkin_names_completed(
                folio_record
            ),
            reservations=reservations,
        )
    ...
    def _check_room_addresses(self, folio_record):
        ...
        is_all_rooms_same_addr = all(
            ...
            and (
                ...
                or room.preferred_room_id.state_id
                != folio_record.pms_property_id.state_id
    ...
    def get_folio_payment_link(self, folio_id, folio_payment_link_search_param):
        ...
        if not folio.exists():
            raise MissingError(_("Folio not found"))
        pms_api_check_access(user=self.env.user, records=folio)
        payment_link = self._generate_payment_link(
            folio, folio_payment_link_search_param.amount
        )
    ...
    def adjust_board_services_input(self, pms_folio_info):
        ...
        # add service to reservations which have boardServiceId and not services
        # with field isBoardService = True
        if pms_folio_info.reservations:
            for reservation in pms_folio_info.reservations:
                res_has_bs = False
                if (
