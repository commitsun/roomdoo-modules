    ...
    def autoinvoice_folio(self, folio, delay_post=False):
        try:
            with self.env.cr.savepoint():
                ...
                # Discard zero-amount invoices generated by _create_invoices.
                # When a folio has several partner_invoice_ids and one of them
                # only owns zero-priced lines (e.g. line_notes added after the
                # original invoicing for late reservations), _create_invoices
                # still produces an empty invoice for that partner. Posting it
                # serves no purpose and clutters accounting.
                zero_invoices = invoices.filtered(lambda inv: not inv.amount_total)
                if zero_invoices:
                    invoices -= zero_invoices
                    zero_invoices.sudo().unlink()
                downpayments = folio.sale_line_ids.filtered(
                    lambda r: r.is_downpayment and r.qty_invoiced > 0
                )
