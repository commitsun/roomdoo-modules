...
from odoo.addons.roomdoo_fastapi.schemas.user import AvailabilityRuleField, ChangePasswordInput, UserEmailInput, ResetPasswordInput
...
async def change_password(password_item: ChangePasswordInput, env: Annotated[Environment, Depends(AuthJwtOdooEnv(validator_name="api_pms"))]):
    user = env.user
...
async def send_mail_reset_password(userEmail: UserEmailInput, env: Annotated[Environment, Depends(odoo_env)]):
    ...
    if user:
        ...
        template.with_context(app_url=app_url).send_mail(
...
async def reset_password(reset_pass_input: ResetPasswordInput, env: Annotated[Environment, Depends(odoo_env)]):
